...
## Wave 1: item art & rune glyphs

- **Items** (`assets/items/*.png`, 128×128, transparent): shortsword, torch, bread, waterflask, ironkey, scroll. Generated by `tools/gen-items/` as stylized "painted icon" compositions — a chunky readable style, not final art. Since M0.11, drawn as foreshortened/grounded compositions rather than front-on standing silhouettes (sword and torch lie diagonally; every item gets real directional light-to-dark shading and a baked soft ground-contact shadow, `groundShadow()`/`directionalShading()` in `tools/gen-items/generate.mjs`) so the art itself sells "resting on the ground," on top of the game-side switch from billboard sprites to flat grounded quads (ARCHITECTURE.md).
